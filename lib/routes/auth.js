...
router
  ...
  .put('/users/:id/roles/:role', ensureAuth(), ensureRole(), (req, res, next) => {
    const object = {$addToSet: {roles: req.params.role}}
    User.findByIdAndUpdate(req.params.id, object)
      .then(user => res.json(user))
      .catch(next);
  })

  .delete('/users/:id/roles/:role', ensureAuth(), ensureRole(), (req, res, next) => {
    const object = {$pull: {roles: req.params.role}}
    User.findByIdAndUpdate(req.params.id, object)
      .then(user => res.json(user))
      .catch(next);
  })

  .get('/users', ensureAuth(), ensureRole(), (req, res, next) => {
    User.find()
